...
const getWeather = (location: string) =>
  Effect.gen(function* () {
    ...
    if (!geocodingData.results?.[0]) {
      return yield* Effect.fail(new Error(`Location '${location}' not found`));
    }
...
export class WeatherToolService extends Effect.Service<WeatherToolService>()(
  "app/WeatherToolService",
  {
    effect: Effect.gen(function* () {
      ...
      const weatherTool = createTool({
        ...
        execute: (inputData) =>
          Runtime.runPromise(
            runtime,
            getWeather((inputData as { location: string }).location),
          ),
      });
